...
Last updated: 2026-07-30 (all sessions through ADR-002 restructure + live E2 verification + open-gaps-plan GAP-1 through GAP-23 catalogued). File paths updated: `fabric-audit-agent-py/` renamed to `fabric-audit-mcp/`; all business logic moved to `fabric-audit-agent-app/fabric_audit_agent/`. See ADR-002 at `fabric-audit-agent-app/docs/decisions/ADR-002-restructure-core-into-agent-app.md`.

**Restructure completed (2026-07-30, ADR-002 — executed as Option A):** `fabric-audit-agent-py/` renamed to `fabric-audit-mcp/` and stripped to `mcp_server.py` + `data_agent.py` only. The `fabric_audit_agent/` core package — **including `tools.py`** — moved into `fabric-audit-agent-app/`. `tools.py` stays in core (not the MCP satellite) because `create_tool_definitions` is shared business logic imported by BOTH the MCP server (`mcp_server.py`) AND the agent brain (`agent_server/investigator.py`); putting it in the MCP package would force the agent app to import `fabric_audit_mcp`, breaking the one-way `MCP → fabric_audit_agent` rule. The MCP server imports `create_tool_definitions` from `fabric_audit_agent.tools` via the installed wheel. No function extraction was needed (`watch_run → tools` is core→core, so there was never a real circular dependency; `tools.py`/`job.py`/`watch_run.py` are byte-identical to pre-restructure). Databricks Jobs moved to agent app `databricks.yml`; MCP keeps `app.yaml`. Tests split by the one-way rule: the 4 files exercising `mcp_server`/`data_agent` (`test_entrypoints`, `test_glue`, `test_mcp_server`, `test_mcp_tools`) stay in the MCP repo; the rest (incl. the tools-only tests) live in the agent app repo. `fixtures/` duplicated into both repos (both `tools.py` and `entrypoints.py` resolve `_BASE/fixtures`). Both suites pass post-restructure (agent app 1421 + 55 subtests; MCP 257).
