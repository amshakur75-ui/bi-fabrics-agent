...
# There used to be a "self-contained port kept in sync with fabric_audit_agent.export" here. It was
# not in sync: when the fabrication bug was fixed, the fix landed in this copy only, so the chat
# table stopped inventing addresses while both exports kept inventing them -- and the exports are
# the artifact people forward to other people. The test that asserted the fix imported only this
# copy, so it passed against two unfixed ones. One implementation now, in the core.
from fabric_audit_agent.identity_display import (  # noqa: F401  (re-exported for callers/tests)
    NEWELL_EMAIL_DOMAIN as _NEWELL_EMAIL_DOMAIN,
    _BARE_UPN_LOCAL_PART, _GUID_SHAPE, normalize_executing_user_display)
