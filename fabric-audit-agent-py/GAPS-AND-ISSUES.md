...
Last updated: 2026-07-27 (post formula-validation session + Item History tab export pass + external FUAM/docs validation pass); reopened C2 + added N22/N23 on 2026-07-29 (live-transcript pass); D1 Option 1 confirmed closed + full measure catalog added 2026-07-29 (composite-model pass); Phase 1 low-risk batch implemented 2026-07-29 (A1/A2/B5/N17/N10/N7/N3/A3/N22 fixed or confirmed done; N18 confirmed real and more nuanced; N19/N21 confirmed moot); **C2 + N15 FIXED 2026-07-29 evening (Task 1+2 architecture migration): prompt + tool loop moved OUT of the MCP package INTO the chat app per ADR-001, canonical single home now `fabric-audit-agent-app/agent_server/system_prompt.py`; `fabric_audit_agent/agent/system_prompt.py` + `agent/loop.py` + `agent/investigator.py` + `agent/scripted_client.py` deleted; agent-case eval (score_agent_case, agent_cases.json) moved alongside; deprecated `fabric-audit-agent-py/app/agent.py` legacy scaffold deleted; obsolete `test_prompt_parity.py` removed. 1,145 package tests + 114 app tests + 35/35 agent-case eval golden suite all green.** **Post-Phase-9 sprint (2026-07-30):** delivery infrastructure (Teams webhook + SMTP email) removed entirely — `delivery_teams.py`, `delivery_email.py`, `teams_card.py`, `PlainJsonHttp` deleted; Phase 10 (Entra bot identity) owns all delivery. Cadences updated: Tier 2 every 5 min (was 15), Tier 1 hourly (was daily), heartbeat 20 min (was 60). P0 fixed: audit_findings wired end-to-end (PySpark schema fix for `createDataFrame`). Investigation enrichment: trend window 24 (was 7) + firstSeenAt, normalityHint on all Tier 2 triggers, monthly baseline bucketing, 4 new system prompt rules. Both apps redeployed and running. 1,538 MCP tests + 113 app tests all green.
