...
export function mergeConfig(overrides = {}) {
  ...
  for (const domain of Object.keys(DEFAULT_CONFIG)) {
    out[domain] = { ...DEFAULT_CONFIG[domain], ...(overrides[domain] ?? {}) };
  }
  // carry through any unknown domains the caller added
  for (const k of Object.keys(overrides)) if (!out[k]) out[k] = overrides[k];
  return out;
}
